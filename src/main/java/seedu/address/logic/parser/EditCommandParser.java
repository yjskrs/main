...
public class EditCommandParser implements Parser<EditCommand> {
    ...
    public EditCommand parse(String args) throws ParseException {
        ...
        if (argMultimap.getValue(PREFIX_TITLE).isPresent()) {
            editPersonDescriptor.setTitle(
                ParserUtil.parseTitle(argMultimap.getValue(PREFIX_TITLE).get()));
        }
        ...
        if (argMultimap.getValue(PREFIX_CREDITS).isPresent()) {
            editPersonDescriptor.setCredits(
                ParserUtil.parseCredits(argMultimap.getValue(PREFIX_CREDITS).get()));
        }
        if (argMultimap.getValue(PREFIX_MEMO).isPresent()) {
            editPersonDescriptor.setMemo(
                ParserUtil.parseMemo(argMultimap.getValue(PREFIX_MEMO).get()));
        }
        if (argMultimap.getValue(PREFIX_SEMESTER).isPresent()) {
            editPersonDescriptor.setSemester(
