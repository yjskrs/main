...
(For instance, the _Computer Science_ course (as shown in _Figure 1_ below) has the various
*modules*, highlighted in blue, and degree *requirements*, boxed in red.)
...
for those invidual degree requirements.
...
* _Course_ (<<Course>>) is indicated in [blue]#blue#
* _Degree Requirements_ (<<Degree Requirements>>) is indicated in [red]#red#
* _Modules_ (<<Modules>>) is indicated in [yellow]#yellow#
...
Under a course are the various degree requirements, such as the ones shown in _Figure 2_ above;
_Computer Science Foundation_, _Mathematics and Sciences_, and _Unrestricted Electives_.
