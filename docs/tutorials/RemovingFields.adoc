...
In this tutorial, we'll do exactly just that and remove the `memo` field from `Person`.
...
The `memo` field in `Person` is actually an instance of the `seedu.memo.model.module.Address` class.
...
. Safe delete the field `memo` in `EditPersonDescriptor`
...
For example, removing the `memo` field from the `Person` class will require you to modify its constructor.
...
For example, there is a `$memo` in each `PersonCard` that has not been removed nor identified.
