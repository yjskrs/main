...
The `VersionedAddressBook` will be initialized with the initial courseInfo book state, and the `currentStatePointer` pointing to that single memo book state.

image::UndoRedoState0.png[]

<<<<<<< HEAD Step 2. The user executes `delete 5` command to delete the 5th module in the courseInfo book.
The `delete` command calls `Model#commitAddressBook()`, causing the modified state of the courseInfo book after the `delete 5` command executes to be saved in the `addressBookStateList`, and the `currentStatePointer` is shifted to the newly inserted courseInfo book state.

image::UndoRedoState1.png[]

Step 3. The user executes `add n/David ...` to add a new module.
The `add` command also calls `Model#commitAddressBook()`, causing another modified address courseInfo book state to be saved into the `addressBookStateList`.
...
The `undo` command will call `Model#undoAddressBook()`, which will shift the `currentStatePointer` once to the left, pointing it to the previous courseInfo book state, and restores the courseInfo book to that state.

image::UndoRedoState3.png[]

[NOTE]
If the `currentStatePointer` is at index 0, pointing to the initial courseInfo book state, then there are no previous courseInfo book states to restore.
...
[NOTE]
If the `currentStatePointer` is at index `addressBookStateList.size() - 1`, pointing to the latest courseInfo book state, then there are no undone courseInfo book states to restore.
...
Commands that do not modify the courseInfo book, such as `list`, will usually not call `Model#commitAddressBook()`, `Model#undoAddressBook()` or `Model#redoAddressBook()`.
Thus, the `addressBookStateList` remains unchanged.

image::UndoRedoState4.png[]

Step 6. The user executes `clear`, which calls `Model#commitAddressBook()`.
Since the `currentStatePointer` is not pointing at the end of the `addressBookStateList`, all courseInfo book states after the `currentStatePointer` will be purged.
...
[none]
* 2a.
The courseInfo name is not provided.
