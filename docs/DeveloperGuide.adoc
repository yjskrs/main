...
As per the Model diagram xref:figure-a-anchor[above], there is only one `CourseBook` in the system.

A `CourseBook` represents all information related to helping a user track her graduation
requirements, including the following:

** One `UniqueModuleList`, consisting of all xref:figure-b-anchor[`Modules`] in the system,
which may or may not be mapped to any (degree) xref:figure-c-anchor[`Requirement(s)`]
** One `UniqueRequirementList`, consisting of all (degree) xref:figure-c-anchor[`Requirements`]
...
In the implementation of the course feature which 'houses' the various xref:figure-b-anchor[`Requirements`]
and the xref:figure-c-anchor[`Modules`] mapped under those requirements, a `CourseInfo` class is
...
because a user might not even have a course in the first place. This occurs when the
application is started out in a 'blank' state, with no initial or sample data.

The following two sections would detail the `Credits` and `Semesters` classes and their design.

Thereafter, the next two sections would attempt to explain the mechanics of the two crucial static
methods `computeCredits(List<Requirement>)` and `computeCap(List<Requirement>)`.

Finally, the last few sections would be dedicated to elaborating how the various
course commands; `course set`, `course edit`, `course achieve`, and `course delete` works.
...
When a user edits name of a course, the user has to specify the prefix `n/` for `Name` and/or `s/`
prefix for `Semesters`.
