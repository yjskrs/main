...
As per the Model diagram xref:figure-a-anchor[above], a `CourseBook` contains and and only one course information.
...
This course information represents important details related to a course such as,
the name of the course, the current cap of the student (or user), the total credits
(MCs) fulfilled/required, and semesters left before he/she could graduate.

As per the the diagram above (_Figure 12_), these information is encapsulated and represented by the
`CourseInfo` class which should only have one `Name`, one `Cap`, one `Credits` and
one `Semesters`.

The course related commands that can be called are:

* `course set` - sets the name and semesters of existing course in the course book
* `course edit` - edits name and/or semesters of existing course in the course book
(more details in <<Course Edit>>
* `course delete` - deletes existing course and all information in the course book
* `course achieve` - calculates CAP needed to maintain each semester to achieve overall CAP target
