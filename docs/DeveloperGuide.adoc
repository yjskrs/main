...
When the user adds a requirement, the user has to specify two prefixes: `n/` for name and `u/` for credits value (number of credits needed to fulfill for the requirement).
...
* Check if a requirement with the same name already exists in the course book.
